...
//#include "glad.h"
//#undef APIENTRY
#include "../Header/RendererOpenGL33.h"
...
Renderer::OGL33GraphicsDevice* gDevice = 0;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow) {
	...
	gDevice = Renderer::CreateOpenGL33GraphicsDevice(hwnd);

	ShowWindow(hwnd, SW_SHOW);
	UpdateWindow(hwnd);
	gApplication->Initialize(gDevice);

	DWORD lastTick = GetTickCount();
	MSG msg;

	Renderer::IRasterState* state = gDevice->MakeRasterState();
	state->SetClearColor(0.5f, 0.6f, 0.7f, 1.0f);
	gDevice->SetDefaultRasterState(state);
	gDevice->DestroyRasterState(state);
	state = 0;
	...
	while (true) {
		...
		if (gApplication != 0) {
			...
			if (gDevice != 0) {
				gDevice->BeginFrame();
				gDevice->SetViewport(0, 0, clientWidth, clientHeight);
			}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_DESTROY:
		...
		if (gDevice != 0) {
			...
			//ReleaseDC(hwnd, hdc);
			DestroyOpenGL33GraphicsDevice(gDevice);
			gDevice = 0;
